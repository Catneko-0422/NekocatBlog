...
[Astro](https://astro.build) で構築された高度な機能と美しいデザインを備えた、モダンで機能が豊富な靜的ブログテンプレート。
...
> **🎯 スマート解像度アルゴリズム** - デバイスの解像度に応じてレイアウトをインテリジェントに適応、あらゆるデバイスでの閲覧體験を提供します。
...
- **構成駆動型のコンポーネントの読み込み:** サイドバーコンポーネントを再構築し、完全に構成ベースのコンポーネントの読み込みメカニズムを実裝しています
- **統合コントロールの切り替え:** 音楽プレーヤーとお知らせのコンポーネントを獨立した有効化切り替えを削除、sidebarLayoutConfig を通じた統合コントロールを実現しています
- **適応型レスポンシブレイアウト:** コンポーネントはレスポンシブレイアウトに対応しており、デバイスの種類に応じた表示を自動調整します

### 📐 レイアウトシステムを最適化

- **動的なサイドバーの位置調整:** 自動なレイアウト適応による、左右サイドバーの切り替えに対応
- **インテリジェントな記事ディレクトリの位置付け:** サイドバーが右側にある場合に自動で左側に移動で、より良い読書體験を提供します
...
- [x] KaTeXレンダリングによる**數式サポート**
- [x] PhotoSwipeギャラリー統合による**畫像最適化**
...
3. **ブログを設定：**
   ...
   - サイト情報、テーマカラー、バナー畫像、ソーシャルリンクを更新
   - 機能ページの機能を設定
...
- **投稿を編集：** `src/content/posts/`內のファイルを修正
- **特別ページをカスタマイズ：** `src/content/spec/`內のファイルを編集
- **畫像を追加：** 畫像を`src/assets/`または`public/`に配置

### 🚀 デプロイ

ブログを任意の靜的ホスティングプラットフォームにデプロイ：

- **Vercel：** GitHubリポジトリをVercelに接続
- **Netlify：** GitHubから直接デプロイ
- **GitHub Pages：** 付屬のGitHub Actionsワークフローを使用
- **Cloudflare Pages：** リポジトリを接続

デプロイ前に、`src/config.ts`の`siteURL`を更新してください。

- **環境変數設定（オプション）：** `.env.example`を參照して設定してください
  **推奨されません**`.env`ファイルをGitにコミットすること。`.env`はローカルデバッグまたはビルドのみで使用する必要があります。クラウドプラットフォームにデプロイする場合、プラットフォームの`環境変數`設定経由で設定することをお勧めします。
...
- **數式：** `$インライン$`と`$$ブロック$$`構文を使用してLaTeX數式を記述
- **コード強調表示：** 行番號とコピーボタン付きの高度な構文強調表示
- **GitHubカード：** `::github{repo="ユーザー/リポジトリ"}`を使用してリポジトリカードを埋め込み

### 🎨 ビジュアル要素

- **畫像ギャラリー：** 畫像表示のための自動PhotoSwipe統合
...
export const siteConfig: SiteConfig = {
  ...
  banner: {
    enable: true,
    src: ["assets/banner/1.webp"], // バナー畫像
    carousel: {
      enable: true,
      interval: 0.8, // 秒
    },
...
Mizukiは、コードとコンテンツを2つの獨立したリポジトリに分けて管理することをサポートしており、チーム協作や大規模プロジェクトに適しています。
...
| 🔧 **分離モード**                   | `ENABLE_CONTENT_SYNC=true`を設定 | チーム協作、プライベートコンテンツ |
...
貢獻は歓迎します！お気軽に問題やプルリクエストを提出してください。
...
- [Pio](https://github.com/Dreamer-Paul/Pio)を使用してかわいいLive2D看板娘プラグインを実裝
...
- **[Firefly](https://github.com/CuteLeaf/Firefly)** - 優れたレイアウトデザインのアイデアを提供していただきありがとうございます。デュアルサイドバーレイアウト、記事の2カラムグリッドレイアウト、およびいくつかのウィジェットのデザインと実裝により、Mizukiのインターフェースがより豊かになりました。
...
このプロジェクトに貢獻してくださったすべてのコントリビューターに感謝します。質問や提案がある場合は、[Issue](https://github.com/LyraVoid/Mizuki/issues)または[Pull Request](https://github.com/LyraVoid/Mizuki/pulls)を提出してください。
