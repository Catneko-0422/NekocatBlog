...
- **環境變數配置（可選）：** 可參照 `.env.example` 來配置
  **不建議**將 `.env` 檔案提交到 Git，`.env` 應該僅在本地調試或構建使用。若要將項目在雲平臺部署，建議通過平臺上的 `環境變數` 配置傳入。
...
本專案基於 Apache 許可證 2.0 - 查看 [LICENSE](./LICENSE) 檔案瞭解詳情。
...
- **[Fuwari](https://github.com/saicaca/fuwari)** by saicaca - 本專案所基於的原始模板。感謝您創建瞭如此漂亮且功能強大的模板。
